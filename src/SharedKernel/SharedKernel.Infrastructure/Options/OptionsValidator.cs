...
namespace SharedKernel.Infrastructure.Options
{
    public static class SharedOptionsValidator
    {
        public static void ValidateMailOptions(IOptions<MailOptions> mailOptions)
        {
            ...
            var validPorts = new int[] { 25, 465, 587, 2525 };

            if (!validPorts.Contains(mailConfig.Port))
                throw new ApplicationException($"{nameof(mailConfig.Port)} in {MailOptions.MailSettings} is misconfigured!");

            if (string.IsNullOrWhiteSpace(mailConfig.Password))
                throw new ApplicationException($"{nameof(mailConfig.Password)} in {MailOptions.MailSettings} is not configured!");
        }
        ...
        public static void ValidateUrlsOptions(IOptions<UrlsOptions> urlsOptions)
        {
            ...
            if(string.IsNullOrWhiteSpace(urlsConfig.Client) || !Uri.IsWellFormedUriString(urlsConfig.Client, UriKind.Absolute))
                throw new ApplicationException($"{nameof(urlsConfig.Client)} in {UrlsOptions.Urls} is not configured!");

            if(string.IsNullOrWhiteSpace(urlsConfig.Idp) || !Uri.IsWellFormedUriString(urlsConfig.Idp, UriKind.Absolute))
