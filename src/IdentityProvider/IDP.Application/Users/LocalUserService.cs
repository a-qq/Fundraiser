...
namespace IDP.Application.Users
{
    internal sealed class LocalUserService : ILocalUserService
    {
        ...
        public async Task<Result<UserDTO, Error>> Login(string userEmail, string password)
        {
            ...
            var userOrNone = await _userRepository.GetUserByEmailAsync(email);
            ...
            var result =
                _passwordHasher.VerifyHashedPassword(userOrNone.Value, userOrNone.Value.HashedPassword, password);
            if (result != PasswordVerificationResult.Success)
                return IDPError.User.LoginFailed();

            if (!userOrNone.Value.IsActive)
                return new Error(
                    "Your account has been suspended! Please contact administrator or your unit system supervisior!");

            _cache.Set(SchemaNames.Authentiaction + userOrNone.Value.Subject, userOrNone.Value,
        ...
        public async Task<Result<bool, Error>> CompleteRegistration(string securityCode, string password)
        {
            ...
            var hashedPassword = HashedPassword.Create(password, _passwordHasher).Value;
        ...
        public async Task<Result<bool, Error>> ChangePassword(string userEmail, string oldPassword, string newPassword)
        {
            ...
            var hashedNewPassword = HashedPassword.Create(newPassword, _passwordHasher).Value;
