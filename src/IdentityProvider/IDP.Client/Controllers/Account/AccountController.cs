...
using IDP.Application.Users.Commands.Login;
...
namespace IDP.Client.Controllers.Account
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            {
                var loginDataOrError = await Mediator.Send(new LoginCommand(model.Email, model.Password));

                if (loginDataOrError.IsSuccess)
                {
                    var loginData = loginDataOrError.Value;
                    await _events.RaiseAsync(new UserLoginSuccessEvent(loginData.Email, loginData.Subject, loginData.Email,
                    ...
                    var isuser = new IdentityServerUser(loginData.Subject)
                    {
                ...
                await _events.RaiseAsync(new UserLoginFailureEvent(
                    model.Email, loginDataOrError.Error, clientId: context?.Client.ClientId));

                ViewData["Error"] = loginDataOrError.Error;
