...
namespace SchoolManagement.Infrastructure.Persistance.Configuration
{
    internal sealed class SchoolConfiguration : IEntityTypeConfiguration<School>
    {
        public void Configure(EntityTypeBuilder<School> b)
        {
            ...
            b.Property(p => p.Name).HasConversion(p => p.Value, p => Name.Create(p).Value).IsRequired()
                .HasMaxLength(500);
            b.Property(p => p.YearsOfEducation).HasConversion(p => p.Value, p => YearsOfEducation.Create(p).Value)
                .IsRequired();
            b.Property(p => p.Description).HasConversion(p => p.Value, p => Description.Create(p).Value)
                .HasMaxLength(3000);
            b.Property(p => p.GroupMembersLimit)
                .HasConversion(p => p.Value, p => GroupMembersLimit.Create(p.Value).Value);
