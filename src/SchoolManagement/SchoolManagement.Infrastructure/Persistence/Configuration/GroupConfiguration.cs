...
namespace SchoolManagement.Infrastructure.Persistence.Configuration
{
    internal sealed class GroupConfiguration : IEntityTypeConfiguration<Group>
    {
        public void Configure(EntityTypeBuilder<Group> b)
        {
            ...
            b.Property(p => p.Number).HasConversion(p => p.Value, p => Number.Create(p, nameof(Number)).Value).HasColumnName("Number")
                .IsRequired();
            b.Property(p => p.Sign).HasConversion(p => p.Value, p => Sign.Create(p, nameof(Sign)).Value).HasColumnName("Sign")
